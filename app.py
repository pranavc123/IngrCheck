...
# Fetch the full list of allergens from Open Beauty Facts
def fetch_allergens():
    url = "https://world.openbeautyfacts.org/allergens.json"
    response = requests.get(url)
    data = response.json()
    allergens = [allergen['name'].lower() for allergen in data['tags']]
    return allergens
